...
/**
 * Reset analysis status for documents in batches
 */
async function resetAnalysisStatusInBatches(documentIds) {
    let totalDeleted = 0
    const batches = []
...
/**
 * Reset analysis status for all documents
 */
async function resetAnalysisStatus() {
    try {
        // Get command line arguments
        const args = process.argv.slice(2)
        const dataSourceId = args[0] // Optional data source ID to filter by

        console.log('Resetting analysis status for documents...')

        // If no data source ID is provided, delete all analysis_status entries
        if (!dataSourceId) {
            console.log('No data source ID provided, resetting all documents')

            const { error, count } = await supabase.from('document_metadata').delete().eq('field_name', 'analysis_status')

            if (error) {
                throw new Error(`Failed to delete analysis status metadata: ${error.message}`)
            }

            console.log(`Successfully reset analysis status for ${count || 0} documents`)
            return
        }
        ...
        // Process in batches
        const totalDeleted = await resetAnalysisStatusInBatches(documentIds)

        console.log(`Successfully reset analysis status for ${totalDeleted} documents`)
    } catch (error) {
