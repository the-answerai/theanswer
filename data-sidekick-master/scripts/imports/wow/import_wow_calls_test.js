...
const BATCH_SIZE = 25 // Increased batch size for full import
...
async function processBatch(batch, sourceId) {
    ...
    for (const record of batch) {
        try {
            ...
            console.log('Document record prepared:', documentRecord.id)
...
async function main() {
    try {
        ...
        if (!researchViewId) {
            ...
            console.error('Usage: node import_wow_calls_test.js <research_view_id>')
            process.exit(1)
        }
        ...
        await new Promise((resolve, reject) => {
            fs.createReadStream(WOW_CALLS_CSV)
                .pipe(csv())
                .on('data', (data) => {
                    console.log('CSV record read:', Object.keys(data))
                    records.push(data)
                })
                .on('end', () => {
