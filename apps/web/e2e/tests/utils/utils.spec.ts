...
test.describe('Test DB utilities', () => {
    ...
    test('Scenario 1: login user with OpenAI assigned', async () => {
        const resetResponse = await axios.post(`${TEST_SERVER_URL}${RESET_PATH}`)
        expect(resetResponse.status).toBe(204)
    ...
    test('Scenario 2: login user with Exa assigned', async () => {
        const resetResponse = await axios.post(`${TEST_SERVER_URL}${RESET_PATH}`)
        expect(resetResponse.status).toBe(204)
    ...
    test('Scenario 3: login user with both credentials assigned', async () => {
        const resetResponse = await axios.post(`${TEST_SERVER_URL}${RESET_PATH}`)
        expect(resetResponse.status).toBe(204)
