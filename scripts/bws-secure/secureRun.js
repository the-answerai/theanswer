...
async function setupEnvironment(options = { isPlatformBuild: false }) {
  ...
  try {
    ...
    let shouldProcessAllProjects = false;
    ...
    if (process.env.SITE_NAME) {
      ...
      // Handle the case when both SITE_NAME and BWS_PROJECT are not available in platform environments
      log('info', `Neither SITE_NAME nor BWS_PROJECT found - will attempt to update all projects`);
      shouldProcessAllProjects = true;
    }
    ...
    if (projectsToUse.length > 1 && !shouldProcessAllProjects) {
      log(
        'info',
    ...
    } else if (shouldProcessAllProjects) {
      log('info', `Will process all ${projectsToUse.length} projects for platform deployment`);
    }
