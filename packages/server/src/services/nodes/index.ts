...
const getSingleNodeAsyncOptions = async (nodeName: string, requestBody: any, user: IUser | undefined): Promise<any> => {
    try {
        const appServer = getRunningExpressApp()
...
const executeCustomFunction = async (requestBody: any, user: IUser) => {
    const appServer = getRunningExpressApp()
    const executeData = {
        appDataSource: appServer.AppDataSource,
        componentNodes: appServer.nodesPool.componentNodes,
