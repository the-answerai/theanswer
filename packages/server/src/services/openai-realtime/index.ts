...
const getAgentTools = async (user: IUser, chatflowid: string): Promise<any> => {
    try {
        const agent = await buildAndInitTool(user, chatflowid)
...
const executeAgentTool = async (
    ...
    try {
        const agent = await buildAndInitTool(user, chatflowid, chatId, apiMessageId)
        const tools = agent.tools
