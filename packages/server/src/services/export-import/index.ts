...
const importData = async (importData: ExportData, user: IUser) => {
    ...
    const updateUserAndOrg = (item: any) => ({
        ...item,
        userId: user.id,
        organizationId: user.organizationId
    })

    if (importData.AgentFlow.length > 0) importData.AgentFlow = importData.AgentFlow.map(updateUserAndOrg)
    if (importData.AgentFlowV2.length > 0) importData.AgentFlowV2 = importData.AgentFlowV2.map(updateUserAndOrg)
    if (importData.AssistantCustom.length > 0) importData.AssistantCustom = importData.AssistantCustom.map(updateUserAndOrg)
    if (importData.AssistantFlow.length > 0) importData.AssistantFlow = importData.AssistantFlow.map(updateUserAndOrg)
    if (importData.AssistantOpenAI.length > 0) importData.AssistantOpenAI = importData.AssistantOpenAI.map(updateUserAndOrg)
    if (importData.AssistantAzure.length > 0) importData.AssistantAzure = importData.AssistantAzure.map(updateUserAndOrg)
    if (importData.ChatFlow.length > 0) importData.ChatFlow = importData.ChatFlow.map(updateUserAndOrg)
    if (importData.ChatMessage.length > 0) importData.ChatMessage = importData.ChatMessage.map(updateUserAndOrg)
    if (importData.ChatMessageFeedback.length > 0) importData.ChatMessageFeedback = importData.ChatMessageFeedback.map(updateUserAndOrg)
    if (importData.CustomTemplate.length > 0) importData.CustomTemplate = importData.CustomTemplate.map(updateUserAndOrg)
    if (importData.DocumentStore.length > 0) importData.DocumentStore = importData.DocumentStore.map(updateUserAndOrg)
    if (importData.DocumentStoreFileChunk.length > 0)
        importData.DocumentStoreFileChunk = importData.DocumentStoreFileChunk.map(updateUserAndOrg)
    if (importData.Execution.length > 0) importData.Execution = importData.Execution.map(updateUserAndOrg)
    if (importData.Tool.length > 0) importData.Tool = importData.Tool.map(updateUserAndOrg)
    if (importData.Variable.length > 0) importData.Variable = importData.Variable.map(updateUserAndOrg)
