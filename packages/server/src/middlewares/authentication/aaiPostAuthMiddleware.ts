...
/**
 * Populate workspace data for req.user
 * Returns workspace context required by Flowise 3.0.11 LoggedInUser type.
 */
async function populateWorkspaceData(
    AppDataSource: DataSource,
    user: User,
    organizationId: string
): Promise<Partial<{
    ...
    // Use first workspace as active (prefer Default Workspace)
    const defaultWs = workspaceUsers.find((wu) => wu.workspace?.name === 'Default Workspace')
    const activeWu = defaultWs || workspaceUsers[0]
    ...
    try {
        ...
        console.warn('[AAI Post-Auth] Error fetching owner role, using name comparison:', error)
        isOrganizationAdmin = workspaceUsers.some((wu) => wu.role?.name === GeneralRole.OWNER)
    }
