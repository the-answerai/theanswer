...
Operators can verify the entire pipeline (config resolution, credential lookup, live Fiddler reachability, circuit state) without sending a real chat:
...
`credentialSource` will be one of `config_credential_id`, `workspace`, `organization`, `env`, or `none`. `healthCheck.reason` is a stable enum (`ok`, `disabled`, `no_credentials`, `auth_error`, `api_error`, `timeout`, `circuit_open`, `network_error`, `unexpected`) suitable for alerting.
