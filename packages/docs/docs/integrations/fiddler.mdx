...
-   **Configurable failure semantics** with explicit fail-open or fail-closed posture per organization or chatflow
-   **Diagnose wiring at any time** via the `/api/v1/guardrails/selftest` admin endpoint
-   **Pilot enforcement safely** with observability-only shadow mode that records violations without blocking
-   **Resilient by design** with circuit breakers, connection pooling, and &lt;150ms latency
...
<div style={{ margin: '1rem 0' }}>
    <AskAlphaButton
        ...
        context={{
            ...
            integration: 'Fiddler',
            question: 'Can I answer any questions about Fiddler failure modes and what happens when the API is unavailable?'
        }}
    />
</div>

Guardrails that silently bypass enforcement when something goes wrong are not guardrails — they are telemetry. AnswerAgentAI exposes the failure posture as a first-class configuration so an admin can choose how the platform reacts when Fiddler cannot be evaluated (bad API key, network outage, upstream timeout, circuit open, no credential resolved).

### Fail-Open vs Fail-Closed

| Failure scenario | Fail-Open (default) | Fail-Closed |
|---|---|---|
| Real safety violation (200 + score) | Block with HTTP 400 | Block with HTTP 400 |
| Bad API key (401 from Fiddler) | Request continues, amber banner in chat, structured error log | Request returns HTTP 503, no LLM call made |
| Missing credential | Request continues, amber banner, structured warn log | Request returns HTTP 503 |
| Fiddler timeout / 5xx | Request continues, amber banner | Request returns HTTP 503 |
| Circuit breaker open | Request continues, amber banner | Request returns HTTP 503 |
| Guardrails disabled by config | Request continues, no banner | Request continues, no banner |

**Default is `open`** so existing deployments see no behavior change until an operator explicitly opts in to strict mode.

### Configuration Hierarchy

`failureMode` follows the standard env → organization → chatflow hierarchy:

1. **Environment variable** — `FIDDLER_FAILURE_MODE=open|closed` (default `open`)
2. **Organization config** — set in the admin Guardrails Settings page (`organizationConfig.guardrails.failureMode`)
3. **Chatflow override** — set in the per-chatflow Guardrails tab (`chatbotConfig.guardrails.failureMode`)

Each layer overrides the previous. A single sensitive chatflow can be `closed` while the rest of the organization is `open`.

### What End Users See

-   **Fail-open + degraded:** an amber banner above the chat response: _"Safety checks were unavailable for this message. The response was generated without safety checks."_ Health detail (reason, HTTP status, latency) is shown in the existing 🛡️ Guardrails Validation accordion.
-   **Fail-closed + degraded:** the prediction returns HTTP 503 with the body `Safety checks are temporarily unavailable (reason: …). Please try again in a moment.` No partial LLM output is delivered.
-   **Healthy or skipped (disabled):** unchanged from before — the green 🛡️ Guardrails Validation accordion appears for healthy validations and nothing renders when guardrails are disabled.

### Observability-Only (Shadow) Mode

For a safe pilot before flipping enforcement on in production:

-   Toggle **Observability-Only** in the admin Guardrails Settings page, or set `FIDDLER_OBSERVABILITY_ONLY=true`.
-   When enabled, even **real** Fiddler 200-OK violations are downgraded from `block` to `warn`. Violations are still recorded in `chat_message.guardrails_metadata` for analytics, dashboards, and Langfuse traces, but no request is blocked.
-   Use this to measure violation volume, calibrate thresholds, and confirm zero false-positives before going hot.

### Diagnostic Selftest Endpoint

Operators can verify the entire pipeline (config resolution, credential lookup, live Fiddler reachability, per-endpoint plan capabilities, circuit state) without sending a real chat:

```bash
curl -H "Authorization: Bearer $YOUR_API_KEY" \
  "https://your-deployment/api/v1/guardrails/selftest?chatflowId=<id>"
```
...
```json
{
  ...
  "config": {
    ...
    "faithfulness": { "enabled": false, "threshold": 0.005, "action": "warn" }
  },
  ...
  "capabilities": {
    "safety":       { "ok": true,  "degraded": false, "reason": "ok",          "latencyMs": 287 },
    "pii":          { "ok": true,  "degraded": false, "reason": "unsupported", "httpStatus": 404 },
    "faithfulness": { "ok": true,  "degraded": false, "reason": "ok",          "latencyMs": 312 }
  },
  "circuitState": "CLOSED",
  "notes": [
...
`credentialSource` will be one of `config_credential_id`, `workspace`, `organization`, `env`, or `none`. `healthCheck.reason` and `capabilities.<stage>.reason` are a stable enum (`ok`, `disabled`, `no_credentials`, `auth_error`, `api_error`, `timeout`, `circuit_open`, `network_error`, `unsupported`, `unexpected`) suitable for alerting.

## Fiddler Plan Tiers (Freemium vs Paid)

Not every Fiddler endpoint is included in every plan tier. The freemium guardrails API ships with safety enabled, but PII detection (`/v3/guardrails/sensitive-information`) and faithfulness (`/v3/guardrails/ftl-response-faithfulness`) require a paid Fiddler subscription. When the runtime calls an endpoint that is not included in the caller's plan, Fiddler responds with HTTP 404 and a body indicating the guardrail is not supported.

AnswerAgentAI distinguishes "Fiddler said no" (plan-tier capability gap) from "Fiddler can't be reached" (degradation):

-   **Capability gap (`reason: "unsupported"`)** — The API answered, the plan just doesn't include the endpoint. The runtime caches the unsupported state per `(api_key, endpoint)` so subsequent calls short-circuit without hitting the network or tripping the circuit breaker. **No per-message banner is shown** because the situation is permanent and configuration-level — not a runtime outage. Admins see it once on the Guardrails Settings page and via `selftest`.
-   **Outage (`reason: "auth_error" | "timeout" | "api_error" | "circuit_open" | "network_error"`)** — Fiddler is unhealthy. Behavior follows `failureMode` (open: per-message banner, closed: HTTP 503).

**Plan capability matrix in the admin UI.** When a Fiddler credential is connected, the Guardrails Settings page renders a row of chips showing the live capability for each endpoint — green ✓ for available, amber − for not included in your plan, red ! for actually degraded. A "Recheck" button re-runs the probe.

**What the user sees with a freemium key.** Safety still runs; PII and faithfulness silently skip. There is no per-message warning banner — chats look identical to a fully-supported plan. Violations metadata records `reason: "unsupported"` for the skipped sub-stage so audit/Langfuse traces remain accurate.

### Credential Visibility

Guardrail credentials are saved with `Organization` visibility by default. The runtime now correctly resolves credentials in this priority order:

1. Explicit `credentialId` referenced in the chatflow or organization config (workspace-owned OR Organization/Platform-visible)
2. Any `fiddlerApi` credential owned by the chatflow's workspace
3. Any `fiddlerApi` credential with `Organization` or `Platform` visibility in the chatflow's organization
4. `FIDDLER_API_KEY` + `FIDDLER_API_URL` environment variables

Multi-workspace organizations are now correctly served by a single org-visible credential.
...
A: The integration's behavior is governed by the configured **failureMode** (`open` by default):

-   **Fail-open:** Invalid credentials, timeouts, 5xx, or network errors → log structured warning, continue the chat, surface an amber degraded banner above the message.
-   **Fail-closed:** Same conditions → return HTTP 503 to the caller, no LLM call made.

**Plan-tier 404s are handled separately** — they are not treated as degradation. If Fiddler responds with `404 NotFound` and "not supported by the freemium guardrails API", the runtime classifies the endpoint as `unsupported`, caches that decision per (api_key, endpoint), and silently skips that guardrail without a banner. See **Fiddler Plan Tiers** above.

**Q: I'm seeing an `HTTP 404` banner in chat. What's wrong?**

A: That banner is shown when Fiddler returns 404 for a reason _other_ than a plan-tier capability gap (which is detected via the response body). The most common causes:

1. Wrong `apiUrl` in the credential — verify it points to your Fiddler guardrails host (e.g. `https://guardrails.cloud.fiddler.ai`).
2. The credential is being resolved from an unexpected source — call `/api/v1/guardrails/selftest?chatflowId=<id>` and inspect `credentialSource`.
3. A Fiddler-side incident — check Fiddler status. The circuit breaker will open after 5 failures and self-heal after 30s.

If you're on a freemium plan and the banner specifically mentions PII / sensitive-information / faithfulness, ensure you're on the latest version — those endpoints are now classified as `unsupported` and skipped silently rather than banner'd.
