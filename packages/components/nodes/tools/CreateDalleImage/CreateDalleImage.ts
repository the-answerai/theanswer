    ...
    async _call(input: string) {
        try {
            ...
            if (this.model === 'gpt-image-1') {
                ...
                const storagePath = await addSingleFileToStorage('image/png', bf, fileName, 'dalle-images')
                const storedFile = storagePath.replace('FILE-STORAGE::', '')
                const baseUrl = process.env.API_BASE_URL || ''
                const imageUrl = `${baseUrl}/api/v1/dalle-image?fileName=${encodeURIComponent(storedFile)}`
                return imageUrl
            } else {
                const text = await res.text()
                return text
