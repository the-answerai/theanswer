...
Integrate Fiddler AI Guardrails into AnswerAgent to automatically validate LLM inputs and outputs for:
- **Safety violations** (11 dimensions: harmful, violent, illegal, sexual, racist, etc.)
- **PII exposure** (15+ types: EMAIL, PHONE_NUMBER, SSN, CREDIT_CARD, etc.)
- **Hallucinations** (faithfulness checks for RAG responses)

**API Coverage**: 100% of Fiddler Guardrails endpoints (3/3: ftl-safety, sensitive-information, ftl-response-faithfulness)
...
**Project Scores**:
- **Effort**: Medium-High (15/25 across 5 phases)
- **Complexity**: Medium-High (3.0/5 average)
- **Risk**: Low-Medium (fail-open design reduces deployment risk)

**MVP Scope:**
- ✅ Phases 1-4, 6: Core infrastructure, input validation, configuration UI, chatflow overrides (100% complete)
- ⏸️ Phase 5: Output validation (deprioritized, fully specified for future implementation)
...
| Endpoint                      | Purpose                         | Status             | MVP Scope                    |
| ----------------------------- | ------------------------------- | ------------------ | ---------------------------- |
| **ftl-safety**                | 11-dimension safety check       | ✅ Fully Specified | Input validation (✅ Complete) |
| **ftl-response-faithfulness** | Hallucination detection for RAG | ✅ Fully Specified | ⏸️ Deprioritized (Phase 5) |
| **sensitive-information**     | PII detection & redaction       | ✅ Fully Specified | Input validation (✅ Complete) |

**Note:** Output validation (Phase 5) is fully specified but deprioritized for MVP. See [status.md](fiddler-guardrails-status.md) for details.
...
-   **Design Principle:** Integration points should be minimal and delegate to service classes rather than embedding business logic inline
